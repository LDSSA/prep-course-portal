'''
Source: https://towardsdatascience.com/generating-fake-data-with-pandas-very-quickly-b99467d4c618
'''
...
def generate_fake_dataframe(size, cols, col_names=None, intervals=None, seed=None):
    ...
    for col, col_name, interval in zip(cols, col_names, intervals):
        ...
        assert (len(interval) == 2 and isinstance(interval, tuple)) or isinstance(
            interval, list), f"This interval {interval} is neither a tuple of two elements nor a list of strings."
        if col in ("i", "f", "d"):
            start, end = interval
        if col == "i":
